...
class TestBaseModel(unittest.TestCase):
    ...
    def test_3_datetime_created(self):
        ...
        diff = b.created_at - datetime.now()
    ...
    def test_5_save_file_content(self):
        """Tests that storage.save() writes correct content to the file."""
        self.resetStorage()
        b = BaseModel()
        b.save()
        with open(FileStorage._FileStorage__file_path,
            ...
            self.assertIn(b.to_dict()["id"], content)
            self.assertIn(b.to_dict()["created_at"], content)
            self.assertIn(b.to_dict()["updated_at"], content)
